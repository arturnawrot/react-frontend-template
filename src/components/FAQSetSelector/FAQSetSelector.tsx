...
const FAQSetSelector: TextFieldClientComponent = (props) => {
  const { path } = props
  const { value, setValue } = useField<string>({ path })
  
  const [sets, setSets] = useState<Array<{ name: string; questions?: any[] }>>([])
  ...
  useEffect(() => {
    const fetchSets = async () => {
      try {
        ...
        const response = await fetch('/api/globals/faqSets', {
          credentials: 'include', // Include cookies for authentication
        })
        
        if (!response.ok) {
          throw new Error(`Failed to fetch FAQ sets: ${response.status} ${response.statusText}`)
        }
        ...
        console.error('Error fetching FAQ sets:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div>
        ...
        {sets.map((set, index) => {
          ...
          return (
            <option key={index} value={set.name}>
              {set.name} ({questionCount} question{questionCount !== 1 ? 's' : ''})
            </option>
          )
        })}
      </select>
      ...
      {!loading && sets.length === 0 && (
        <p style={{ marginTop: '8px', color: '#6b7280', fontSize: '12px' }}>
          No sets available. Create sets in the FAQ Sets global.
        </p>
      )}
    </div>
  )
